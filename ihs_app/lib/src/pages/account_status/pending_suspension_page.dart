...
class PendingSuspensionPageSettings {
  PendingSuspensionPageSettings({required this.registration});
  MobileRegistration registration;
}

class PendingSuspensionPage extends StatefulWidget {
  const PendingSuspensionPage({required this.settings, super.key});
  static const RouteName routeName = RouteName('/pendingSuspension');
  final PendingSuspensionPageSettings settings;

  @override
  PendingSuspensionPageState createState() => PendingSuspensionPageState();
  @override
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DiagnosticsProperty<PendingSuspensionPageSettings>(
        'settings', settings));
  }
}

class PendingSuspensionPageState extends State<PendingSuspensionPage> {
  PendingSuspensionPageState() {
    // mobile = settings.progress.mobile;
    // validMobile = PhoneNumber
    // .isMobile(settings.progress.mobile.toNational());
    settings = widget.settings;
  }
  late final PendingSuspensionPageSettings settings;
  ...
  Widget build(BuildContext context) => NoAppBarScaffold(
        ...
        currentRouteName: PendingSuspensionPage.routeName,
        builder: (context) => buildBody(),
      ));

  Widget buildBody() =>
      const SingleChildScrollView(child: Text('Pending Suspension'));
  ...
  LocalTime getDefaultCustomPeriod() {
    ...
    // round minutes to nearest 5 minutes so the TimePicker shows a
    // selected minute button.
