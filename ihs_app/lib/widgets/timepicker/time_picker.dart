...
class TimePicker extends StatefulWidget {
  ...
  TimePicker(this.heading, this.initialTimeOfDay);

  @override
  State<StatefulWidget> createState() {
    return TimePickerState(initialTimeOfDay);
  }

  static Future<LocalTimeState> show(BuildContext context, String heading, LocalTime timeOfDay) {
    var result = showModalBottomSheet<LocalTimeState>(
        isScrollControlled: true, context: context, builder: (context) => TimePicker(heading, timeOfDay));
...
class TimePickerState extends State<TimePicker> {
  LocalTimeState timeOfDay;

  TimePickerState(LocalTime selected) : timeOfDay = LocalTimeState(selected);

  @override
  Widget build(BuildContext context) {
    return MultiProvider(
        providers: [
          ChangeNotifierProvider<LocalTimeState>(create: (context) => timeOfDay),
          Provider<MoonPhase>(create: (context) => MoonPhase())
        ],
        child: SizedBox(
  ...
  Widget buildTimeEditor() {
    return Row(children: [
      ...
      //     TextFormField(),
      //    TextFormField(),
      Consumer<MoonPhase>(builder: (context, phase, _) => MoonPhase())
    ]);
  }

  Widget buildHourSelection() {
    return Column(children: [
      Align(alignment: Alignment.topLeft, child: NJTextNotice('Select Hour')),
      Row(children: [
        HourButton(label: '12'),
        HourButton(label: '1'),
        HourButton(label: '2'),
  ...
  Widget buildMinuteSelection() {
    return Column(children: [
      Align(alignment: Alignment.topLeft, child: NJTextNotice('Select Minute')),
      Row(children: [
  ...
  Widget buidAMPMSelection() {
    return Column(children: [
      Align(alignment: Alignment.topLeft, child: NJTextNotice('Select AM/PM')),
      Row(children: [
        AMPMButton(label: 'AM'),
        AMPMButton(label: 'PM'),
      ]),
